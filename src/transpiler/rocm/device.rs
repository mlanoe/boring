...
//
// HIP C++ device code emitter.
//
// HIP's device-side syntax is source-compatible with CUDA C by design (that's
// the whole point of HIP) -- `__global__`/`__device__`/`__constant__`/
// `__shared__`, `threadIdx`/`blockIdx`/`blockDim`/`gridDim`, `atomicAdd` &
// friends, `__syncthreads()`, and device-side `printf` all carry the exact
// same names and semantics. This emitter is therefore a near-verbatim clone of
// `cuda::device` — see that module's doc comment for the shared architecture
// notes (auto-barrier `'sync` fields, atomic compound-assign desugaring,
// etc.) — with only the file header/include and the `__int128` portability
...
struct DeviceEmitter {
    ...
    auto_sync: bool,
    // Top-level scalar lets inlined into HIP C++ (not in scope in device code —
...
impl DeviceEmitter {
    ...
    fn emit_program(&mut self, program: &Program) {
        ...
        // generated .hip file, so any kernel body referencing it needs the literal
        ...
        self.line("// Compile to a code object with: hipcc --genco kernels/main.hip -o $OUT_DIR/main.hipfb");
        self.blank();
        self.line("#include <hip/hip_runtime.h>");
        ...
        // string formatting, etc.) that have no HIP C++ equivalent. Emitting
        // every free function unconditionally makes kernels/main.hip fail to
    ...
    fn emit_kernel_decl(&mut self, decl: &KernelDecl) {
        ...
        // These are file-scope globals in HIP, not kernel parameters.
        ...
        for field in &decl.fields {
            if matches!(field.qual, GpuQual::Local) {
                match &field.ty {
                    ...
                    // launch (`launcher.arg(&self.{field})` in rocm/host.rs) -- redeclaring
                    // it here would both collide with that parameter name (a real "already
                    // declared in this scope" error, see `cuda::device`'s identical fix,
                    // confirmed via examples/plasma_metal.br's `var float t`) and discard
                    // the seeded value by resetting it to 0. Nothing to do: the parameter
                    // already provides a thread-local mutable register under this exact name.
                    _ => {}
                }
            }
        }
    ...
    fn expr(&mut self, e: &Expr) -> String {
        match &e.kind {
            ...
            ExprKind::Field(obj, field) => {
                if let Some(call) = self.try_labeled_array_field_access(obj, field) {
                    call
                } else {
            ...
            ExprKind::MethodCall(obj, method, args) => {
                ...
                if matches!(&obj.kind, ExprKind::Var(n) if n == "self") {
                    ...
                    // directly onto HIP C++'s device math functions of the same shape.
                    call
                } else {
...
fn c_type(ty: &Type) -> String {
    match ty {
        ...
        // are a GCC/Clang (and HIP-clang-supported) extension, not standard C/C++ —
        // flagged in a comment since portability to other compilers isn't guaranteed.
        Type::Uint8             => "uint8_t".into(),
        Type::Int8               => "int8_t".into(),
        Type::Int16              => "int16_t".into(),
        Type::Int32              => "int32_t".into(),
        Type::Int64              => "int64_t".into(),
        Type::Int128              => "__int128 /* GCC/HIP-clang extension */".into(),
        Type::Uint16             => "uint16_t".into(),
        Type::Uint32             => "uint32_t".into(),
        Type::Uint64             => "uint64_t".into(),
        Type::Uint128             => "unsigned __int128 /* GCC/HIP-clang extension */".into(),
        ...
        Type::Named(n) => match n.as_str() {
            ...
            "int128"                 => "__int128 /* GCC/HIP-clang extension */".to_string(),
            "uint16"                 => "uint16_t".to_string(),
            "uint32"                 => "uint32_t".to_string(),
            "uint64"                 => "uint64_t".to_string(),
            "uint128"                => "unsigned __int128 /* GCC/HIP-clang extension */".to_string(),
            "bool"                  => "int".to_string(),
...
fn map_builtin_fn(name: &str) -> String {
    match name {
        ...
        // undeclared `float32(x)` function call in the generated HIP C++
        // (confirmed via `boring build --target rocm examples/mandelbrot_gpu.br`
        // + `hipcc`, on real gfx1032 hardware: "use of undeclared identifier
        // 'float32'"). Same bug as CUDA's identical `map_builtin_fn` (this
        // device emitter is a near-verbatim clone of it, per rocm::mod's doc
        // comment) — Metal's own `map_builtin_fn` already carries the
        // equivalent fix. The rest of this arm set mirrors `c_type`'s
        // `Type::Named` cast-name table just above, for the same call-style
        // casts (`uint8(x)`, `bool(x)`, ...) that table already resolves for
        // `x as T`-style casts.
