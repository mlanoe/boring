...
- **`DeviceBuffer<T>`**: wraps a raw `hipMalloc`'d pointer + length + the `Arc<HipStream>` it was allocated on. `Drop` calls `hipFree`. `Clone` does a real device-to-device `hipMemcpyDtoD` — a correctness-preserving fallback for the rare case a resident value is needed twice, not the common path (every real call site moves a buffer instead of cloning it), matching the CUDA backend's identical `CudaSlice::clone()` rationale.
