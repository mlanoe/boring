...
"#);
    assert!(hip.contains("c < 4"), "expected img.width to lower to the literal 4;\ngot:\n{hip}");
    assert!(hip.contains("r < 8"), "expected img.height to lower to the literal 8;\ngot:\n{hip}");
}
...
fn device_auto_sync_barrier_found_when_loop_nested_inside_top_level_if() {
    ...
    let (hip, _) = rocm_codegen("auto_sync_nested_if", r#"
kernel Reduce:
...
fn kernel_touching_struct_method_is_a_hard_build_error() {
    ...
    let bin = env!("CARGO_BIN_EXE_boring");
    let tmp = PathBuf::from(env!("CARGO_TARGET_TMPDIR"))
        .join("rocm_codegen").join("kernel_touching_struct_method");
    let _ = fs::remove_dir_all(&tmp);
    fs::create_dir_all(&tmp).unwrap();
...
"#).unwrap();
    ...
    assert!(
        !tmp.join("test_rocm").join("src").join("main.rs").exists(),
