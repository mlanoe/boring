...
static MATRIXMUL_PIPELINE: std::sync::OnceLock<std::sync::Arc<wgpu::ComputePipeline>> = std::sync::OnceLock::new();
...
struct MatrixMul {
    ...
    c_buf: std::sync::Arc<wgpu::Buffer>,
    c: Vec<f32>,
}
...
impl MatrixMul {
    fn new(device: std::sync::Arc<wgpu::Device>, queue: std::sync::Arc<wgpu::Queue>) -> Self {
        ...
        let c_buf = std::sync::Arc::new(device.create_buffer(&wgpu::BufferDescriptor {
            label: None,
            size: (1024 * std::mem::size_of::<f32>()) as u64,
        ...
        let pipeline = MATRIXMUL_PIPELINE.get_or_init(|| {
    ...
    fn rebuild_bind_group(&mut self) {
        ...
        entries.push(wgpu::BindGroupEntry { binding: 2, resource: self.c_buf.as_entire_binding() });
    ...
    fn copy_a_to_host(&self) -> Vec<f32> {
        __boring_gpu_copy_d2h::<f32>(&self.device, &self.queue, &self.a_buf)
    }
    fn copy_a_to_device(&mut self, data: &[f32]) {
    ...
    fn copy_b_to_host(&self) -> Vec<f32> {
        __boring_gpu_copy_d2h::<f32>(&self.device, &self.queue, &self.b_buf)
    }
    fn copy_b_to_device(&mut self, data: &[f32]) {
        let needed = (data.len() * std::mem::size_of::<f32>()) as u64;
    ...
    fn copy_c_to_host(&self) -> Vec<f32> {
        __boring_gpu_copy_d2h::<f32>(&self.device, &self.queue, &self.c_buf)
    }
    fn copy_c_to_device(&mut self, data: &[f32]) {
        let needed = (data.len() * std::mem::size_of::<f32>()) as u64;
        if self.c_buf.size() != needed {
            self.c_buf = std::sync::Arc::new(self.device.create_buffer(&wgpu::BufferDescriptor {
                label: None,
                size: needed,
                usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
        ...
        __boring_gpu_copy_h2d(&self.device, &self.queue, bytemuck::cast_slice(data), &self.c_buf);
    }
...
fn boring_main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut host_a: Vec<f32> = (0..(size as usize)).map(|__boring_i| { let i = __boring_i as isize; (((i % 100) as f32) / 100.0) }).collect::<Vec<_>>();
    let mut host_b: Vec<f32> = (0..(size as usize)).map(|__boring_i| { let i = __boring_i as isize; (((i % 50) as f32) / 50.0) }).collect::<Vec<_>>();
    let mut k = MatrixMul::new(__boring_gpu_device(), __boring_gpu_queue());
    k.copy_a_to_device(&host_a.iter().map(|&x| x as f32).collect::<Vec<f32>>());
    k.copy_b_to_device(&host_b.iter().map(|&x| x as f32).collect::<Vec<f32>>());
    k.dispatch((2) as u32, (2) as u32, (1) as u32)?;
    ...
    {
        let c = k.copy_c_to_host().iter().map(|&x| x as f32).collect::<Vec<f32>>();
        for i in (0..10) {
            for j in (0..10) {
                println!("C[{}][{}] = {:.4}", i, j, c[(((i * WIDTH) + j)) as usize].clone());
            }
        }
    }
    Ok(())
