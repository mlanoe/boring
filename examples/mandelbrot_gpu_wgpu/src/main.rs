...
static MANDELBROT_PIPELINE: std::sync::OnceLock<std::sync::Arc<wgpu::ComputePipeline>> = std::sync::OnceLock::new();
...
impl Mandelbrot {
    fn new(device: std::sync::Arc<wgpu::Device>, queue: std::sync::Arc<wgpu::Queue>) -> Self {
        ...
        let image_buf = std::sync::Arc::new(device.create_buffer(&wgpu::BufferDescriptor {
            label: None,
            size: 4u64,
        ...
        let pipeline = MANDELBROT_PIPELINE.get_or_init(|| {
    ...
    fn rebuild_bind_group(&mut self) {
        ...
        entries.push(wgpu::BindGroupEntry { binding: 0, resource: self.image_buf.as_entire_binding() });
    ...
    fn copy_image_to_host(&self) -> Vec<f32> {
        __boring_gpu_copy_d2h::<f32>(&self.device, &self.queue, &self.image_buf)
    }
    fn copy_image_to_device(&mut self, data: &[f32]) {
        let needed = (data.len() * std::mem::size_of::<f32>()) as u64;
        if self.image_buf.size() != needed {
            self.image_buf = std::sync::Arc::new(self.device.create_buffer(&wgpu::BufferDescriptor {
                label: None,
                size: needed,
                usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
        ...
        __boring_gpu_copy_h2d(&self.device, &self.queue, bytemuck::cast_slice(data), &self.image_buf);
    }
...
fn colorize(t: f32) -> Vec<isize> {
    if (t >= 1.0) {
        return vec![0, 0, 0];
    }
    let v = (t * 4.0);
    if (v < 1.0) {
        let b = ((v * 255.0) as isize);
        return vec![0, 0, b];
    } else if (v < 2.0) {
        let g = (((v - 1.0) * 255.0) as isize);
        return vec![0, g, 255];
    } else if (v < 3.0) {
        let r = (((v - 2.0) * 255.0) as isize);
        return vec![r, 255, 255];
    } else {
...
fn boring_main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    {
        let flat_image = k.copy_image_to_host().iter().map(|&x| x as f32).collect::<Vec<f32>>();
        for i in (0..total_pixels) {
            let rgb = colorize(flat_image[(i) as usize].clone());
            pixels[((i * 3)) as usize] = rgb[(0) as usize].clone();
            pixels[(((i * 3) + 1)) as usize] = rgb[(1) as usize].clone();
            pixels[(((i * 3) + 2)) as usize] = rgb[(2) as usize].clone();
        }
    }
    let mut ppm_data: Vec<isize> = vec![];
    for b in header_bytes.iter().cloned() {
        ppm_data.push(b);
    }
    for b in pixels.iter().cloned() {
        ppm_data.push(b);
    }
    (std::fs::write(&*("mandelbrot.ppm"), &ppm_data.iter().cloned().map(|b| (b as u8)).collect::<Vec<_>>()))?;
    println!("Image saved as 'mandelbrot.ppm'");
    Ok(())
}
