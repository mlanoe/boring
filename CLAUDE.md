...
[book.md](docs/book.md#mut-composes-into-any-type-position)) — this is what lets `mut`
...
([book.md](docs/book.md#mutable-fields)) — `mutable` (`var`) controls **reassignment**
...
`mut` on a primitive (`int`, `uint`, `float`, `bool`) is a **checker error** — there are no `def` methods on a scalar for `mut` to unlock, and (per [book.md](docs/book.md#fixed-mutable-bindings--mut)) `mut` never silently degrades to `var`. Use `var` for a rebindable scalar. (Retired: the historical "`mut` ≡ `var` for scalars" shortcut this section used to document.)
...
Parameter passing hierarchy (`var` ≥ `mut` ≥ `let`, caller can pass down, never up) is the intended design but is **not currently enforced** — passing a `let` binding into a `var` parameter compiles and runs without error. (Unlike local bindings above, the parameter model itself is a distinct case from local bindings — see [book.md](docs/book.md#mut-vs-var-on-a-struct-parameter)'s "`mut` vs `var` on a struct parameter".)
